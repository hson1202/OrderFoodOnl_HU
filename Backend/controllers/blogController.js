  ...
  try {
    ...
    const [blogs, total] = await Promise.all([
      Blog.find(filter)
        ...
        .select(
          'title titleVI titleEN titleHU slug excerpt author category tags image language readTime featured createdAt publishedAt views status'
        ),
...
export const createBlog = async (req, res) => {
  try {
    ...
    const { title, titleVI, titleEN, titleHU, content, excerpt, author, category, tags, status, featured, language, readTime, slug } = req.body
    ...
    const finalLanguage = language && ['vi', 'en', 'hu'].includes(language) ? language : 'vi'
    
    const blogData = {
      title: title.trim(),
      titleVI: titleVI ? titleVI.trim() : undefined,
      titleEN: titleEN ? titleEN.trim() : undefined,
      titleHU: titleHU ? titleHU.trim() : undefined,
